...
public interface HibernateSessionMethodDAO {
  ...
  /***
   * This method is similar to save method within transaction. Persist method cannot be used outside the
   * transaction. Persist() method wont return any id like save() method
   *
   * It makes a transient instance persistent. However, it doesn't guarantee that the identifier value will be
   * assigned to the persistent instance immediately, the assignment might happen at flush time. The spec
   ...
   * present in the database, update query is executed.
   *
   * We can use saveOrUpdate() without transaction also, but again you will face the issues with mapped
   * objects not getting saved if session is not flushed.
   *
   * Hibernate saveOrUpdate adds the entity object to persistent context and track any further changes. Any
   * further changes are saved at the time of committing transaction, like persist.
   *
   * Here I am using saveOrUpdate method within transaction even though there are no cascading cases here.
   *
   *
   * @param student
   * @return id
